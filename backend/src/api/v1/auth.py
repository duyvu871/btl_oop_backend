...
from src.core.security import get_verified_user
from src.domains.auth.use_cases import AuthUseCase, get_auth_usecase
...
async def login(
    ...
    try:
        result = await auth_use_case.login(db, login_in.email, login_in.password)
        return result
...
async def login_for_access_token(
    ...
    try:
        result = await auth_use_case.login_by_username(db, form_data.username, form_data.password)
        return result
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
